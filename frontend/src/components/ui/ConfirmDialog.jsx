...
export default function ConfirmDialog({ isOpen, title, message, onConfirm, onCancel, confirmLabel = 'Confirmar', danger = false, disabled = false }) {
    ...
    <Portal>
      <div className="fixed inset-0 bg-black/25 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
        <div className="bg-brand-surface rounded-2xl border border-brand-border shadow-card-hover max-w-sm w-full p-6 animate-slide-up">
          ...
          <div className="text-sm text-brand-text-2 mb-6 leading-relaxed">{message}</div>
          <div className="flex gap-3">
            <button onClick={onCancel} className="btn-secondary flex-1">Cancelar</button>
            <button
              onClick={onConfirm}
              disabled={disabled}
              className={`flex-1 ${danger ? 'btn-danger' : 'btn-primary'} ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`}
            >
              {confirmLabel}
            </button>
          </div>
